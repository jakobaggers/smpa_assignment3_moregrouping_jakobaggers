...
impeach %>% 
  group_by(for_impeachment) %>% 
  summarize(mean_pct_bachelor = mean(pct_bachelors))
...
impeach %>% 
  group_by(for_impeachment) %>% 
  summarize(mean_trumpvote = mean(trump_percent))
...
I'm going to look at the pct of nonwhite constituents in the district.
```{r}
...
impeach <-
  impeach %>% 
  mutate(year = substr(date_announced, 0,4),
         month = substr(date_announced, 6, 7))
